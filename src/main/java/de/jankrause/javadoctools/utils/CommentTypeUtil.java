...
public class CommentTypeUtil {
    ...
    public static CommentType deriveCommentType(Tag tag) {
        if(tag.holder().isField()){
            return CommentType.FIELD;
        } else if (tag.holder().isInterface()) {
            return CommentType.INTERFACE;
        } else if (tag.holder().isClass()) {
